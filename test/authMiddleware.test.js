...
describe("verifyToken middleware", () => {
    ...
    it("should return 401 if an operational user try to create a zone or shelf or corridor or storage", async () => {
        req.method = "POST"
        const res = mockResponse()
    ...
    it("should return 401 if an operational user try to updated a a zone or shelf or corridor or storage", async () => {
        req.method = "PUT"
        const res = mockResponse()
        const token = jwt.sign({ id:'000898'}, process.env.JWT_SECRET);
        req.headers = { authorization: `Bearer ${token}` }
